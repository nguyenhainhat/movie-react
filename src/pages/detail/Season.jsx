      ...
      {items.map((item, i) => (
        <div className="season" key={i}>
          <div className="season-item">
            <img src={`${apiConfig.w200Image(item.poster_path)}`} alt={item.name} />
          ...
          <SeasonItem item={item} id={props.id} />
        </div>
      ))}
    </>
  );
          ...
          {episodes.map((item, i) =>
            item.still_path !== null ? (
              ...
              <SwiperSlide key={i}>
                {({isActive}) => (
                  <Link
                    to={`watch/season/${item.season_number}/episodes/${item.episode_number}`}
